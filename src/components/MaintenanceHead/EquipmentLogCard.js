...
export default function EquipmentLogCard({
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 flex flex-col min-w-[320px]">
      {/* Card Header */}
      <div
        className={`flex items-center justify-between px-4 py-3 rounded-t-xl ${statusColor}`}
      >
        <div>
      ...
      <div className="px-4 py-4 flex-1 flex flex-col justify-between">
        <div className="mb-3">
          ...
          <div className="flex items-center text-sm mb-1 justify-between">
            ...
            <span className={`font-medium ${nextServiceColor}`}>
              {nextService}
            </span>
          </div>
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => onViewLogs && onViewLogs(id)}
            className="flex-1 bg-deep_green/90 hover:bg-deep_green text-purewhite px-4 py-2 rounded-md text-sm font-medium flex items-center justify-center gap-1"
          >
            <ClipboardCheck className="w-4 h-4" /> View Logs
          </button>
          {actions.includes("Schedule Service") && (
            <button
              onClick={() => onScheduleService && onScheduleService(id)}
              className="flex-1 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50 flex items-center justify-center gap-1"
            >
