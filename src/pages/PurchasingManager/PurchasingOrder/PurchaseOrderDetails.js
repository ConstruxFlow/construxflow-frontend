...
const PurchaseOrderDetails = () => {
  ...
  const getOrderStatusSteps = () => {
    ...
    const currentStatusIndex = steps.findIndex(step => 
      step.id.toLowerCase() === orderData?.status?.toLowerCase()
    );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-main_dark">{formatDate(orderData.orderDate)}</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div>
                    ...
                    return (
                      <div key={step.id} className="flex flex-col items-center">
                        ...
                        <span className={`mt-2 text-xs font-medium ${
                          step.isCompleted ? 'text-main_dark' : 'text-gray-400'
                        }`}>
                          {step.label}
                        </span>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-light_brown/20 rounded-lg">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      getPaymentStatusColor(orderData.orderPayment?.status)
                    }`}>
                  ...
                  <div className="pt-4 border-t">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm font-semibold">
                        {calculatePaymentProgress(orderData.orderPayment).toFixed(0)}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-green-500 h-2 rounded-full transition-all duration-300" 
                        style={{ width: `${calculatePaymentProgress(orderData.orderPayment)}%` }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="bg-purewhite border border-gray-200 rounded-lg p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-main_dark">TRK-789456123</p>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-web_yellow rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-main_dark">1</span>
                    </div>
                    <div>
                      <p className="font-semibold text-main_dark">Order Created</p>
                      <p className="text-sm text-gray-600">
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-web_yellow rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-main_dark">2</span>
                    </div>
                    <div>
                      <p className="font-semibold text-main_dark">Approved by Manager</p>
                      <p className="text-sm text-gray-600">
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-web_yellow rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-main_dark">3</span>
                    </div>
                    <div>
                      <p className="font-semibold text-main_dark">Deposit Payment Processed</p>
                      <p className="text-sm text-gray-600">
                        {formatDateTime(orderData.orderPayment?.createdDate)} - 9:00 AM
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-web_yellow rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-main_dark">4</span>
                    </div>
                    <div>
                      <p className="font-semibold text-main_dark">Payment Processing</p>
                      <p className="text-sm text-gray-600">
                        {formatDateTime(orderData.orderPayment?.createdDate)} - 11:45 AM
                      </p>
                    </div>
                  </div>
                </div>
