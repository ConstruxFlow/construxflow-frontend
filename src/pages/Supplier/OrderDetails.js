...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/purchasingorder/ponumber/${ponumber}`);
  ...
  const getStatusTimeline = () => {
    ...
    return statuses.map((step) => {
      ...
      if (step.status === 'ordered') {
        ...
        completed = ['dispatched', 'delivered', 'completed'].includes(currentStatus);
  ...
  const handleMarkAsDispatched = async () => {
    if (!window.confirm('Are you sure you want to mark this order as dispatched?')) {
      return;
    }
      ...
      <div className="min-h-screen bg-purewhite font-poppins">
        ...
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-600 text-lg font-semibold mb-2">{error || 'Order not found'}</p>
    ...
    <div className="bg-purewhite min-h-screen font-poppins">
      ...
      <div className="max-w-full mx-auto px-4 sm:px-12 md:px-16 py-10">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-main_dark">Order Details</h1>
        </div>
          ...
          <div className="md:col-span-2 bg-[#f5f4f3] rounded-lg p-6 flex flex-col gap-2">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className="text-main_dark font-medium">
                {orderData.project || orderData.additionalInfo || 'Purchase Order'}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 text-slatebluegray text-sm mb-1">
              ...
              <span>{orderData.supplier?.company_name || orderData.supplier?.name || 'Supplier'}</span>
            </div>
            <div className="flex items-center gap-2 text-slatebluegray text-sm mb-1">
              <FaMapMarkerAlt className="text-deep_green" />
              <span>{orderData.deliveries?.[0]?.location || 'Delivery Location'}</span>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 mt-2">
              <div className="flex items-center gap-2">
                <FaCheckCircle className="text-deep_green" />
                <span className="text-main_dark text-sm font-medium">{formatDate(orderData.orderDate)}</span>
                <span className="text-slatebluegray text-xs">Order Date</span>
              </div>
              <div className="flex items-center gap-2">
                <FaTruck className="text-web_yellow" />
                <span className="text-main_dark text-sm font-medium">{formatDate(orderData.deliveryDate)}</span>
              ...
              <tbody>
                {orderData.materials && orderData.materials.length > 0 ? (
                  orderData.materials.map((item, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-[#f5f4f3]" : "bg-purewhite"}>
                      <td className="px-6 py-4 font-medium">{item.material?.materialName || 'N/A'}</td>
                      <td className="px-6 py-4">{item.material?.materialType || 'N/A'}</td>
                      <td className="px-6 py-4">
                        {item.quantity} {item.material?.unitOfMeasurement || ''}
                      </td>
                      <td className="px-6 py-4">
                        RS {(item.unitPrice || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </td>
                      <td className="px-6 py-4 font-semibold">
                        RS {((item.quantity || 0) * (item.unitPrice || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </td>
                    </tr>
                  ))
                ) : (
              ...
              <tfoot>
                <tr className="bg-light_gray">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-deep_green text-lg">
                    RS {(orderData.subTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              </tfoot>
            </table>
        ...
        <div className="bg-purewhite border border-light_gray rounded-lg p-6 mb-8">
            ...
            {statusTimeline.map((step, idx) => (
              <React.Fragment key={idx}>
                ...
                {idx < statusTimeline.length - 1 && (
                  <div className={`flex-1 h-1 mx-2 rounded hidden sm:block ${
          ...
          {(orderData.status?.toLowerCase() === 'ordered' || orderData.status?.toLowerCase() === 'approved') && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-purewhite border border-light_gray rounded-lg p-6 flex flex-col gap-3">
            <div className="font-semibold text-main_dark mb-2">Delivery Locations</div>
