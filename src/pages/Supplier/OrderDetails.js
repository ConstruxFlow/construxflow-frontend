              ...
              <tbody>
                {order.items.map((item, idx) => (
                  <tr key={item.name} className={idx === 1 ? "bg-[#f5f4f3]" : "bg-purewhite"}>
                    ...
                    <td className="px-6 py-4">RS {item.unitPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    <td className="px-6 py-4 font-semibold">RS {item.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-light_gray">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-red-600 text-lg">
                    RS {order.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              </tfoot>
            </table>
            ...
            {order.statusTimeline.map((step, idx) => (
              <React.Fragment key={step.label}>
                ...
                {idx < order.statusTimeline.length - 1 && (
                  <div className="flex-1 h-1 bg-light_gray mx-2 rounded hidden sm:block" />
                )}
              </React.Fragment>
            ))}
          </div>
